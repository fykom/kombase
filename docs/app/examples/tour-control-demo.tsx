import {
  Tour,
  TourArrow,
  TourClose,
  TourDescription,
  TourFooter,
  TourHeader,
  TourNext,
  TourPrev,
  TourSpotlight,
  TourSpotlightRing,
  TourStep,
  TourStepCounter,
  TourTitle,
} from 'kombase';
import * as React from 'react';
import { Button } from '@/components/ui/button';

export default function TourControlDemo() {
  const [open, setOpen] = React.useState(false);
  const [value, setValue] = React.useState(0);

  const onStepChange = React.useCallback((step: number) => {
    setValue(step);
  }, []);

  const onComplete = React.useCallback(() => {
    setOpen(false);
    setValue(0);
  }, []);

  const onSkip = React.useCallback(() => {
    setOpen(false);
    setValue(0);
  }, []);

  const onTourStart = React.useCallback(() => {
    setValue(0);
    setOpen(true);
  }, []);

  return (
    <div className="flex min-h-100 flex-col items-center justify-center gap-8 p-8">
      <div className="flex flex-col items-center gap-4">
        <h1 className="font-bold text-2xl" id="controlled-title">
          Start Tour
        </h1>
        <div className="flex items-center gap-2">
          <Button id="controlled-start-btn" onClick={onTourStart}>
            Start
          </Button>
          <Button
            disabled={!open || value === 0}
            onClick={() => setValue(Math.max(0, value - 1))}
            variant="outline"
          >
            Prev
          </Button>
          <Button
            disabled={!open || value === 3}
            onClick={() => setValue(Math.min(3, value + 1))}
            variant="outline"
          >
            Next
          </Button>
        </div>
      </div>
      <div className="flex w-full flex-col gap-6">
        <div className="grid grid-cols-2 gap-6">
          <div className="rounded-lg border p-6 text-center" id="controlled-step-1">
            <h3 className="font-semibold">Step 1</h3>
            <p className="text-muted-foreground text-sm">First step in our controlled tour</p>
          </div>
          <div className="rounded-lg border p-6 text-center" id="controlled-step-2">
            <h3 className="font-semibold">Step 2</h3>
            <p className="text-muted-foreground text-sm">Second step with external controls</p>
          </div>
        </div>
        {open && value >= 2 && (
          <div
            className="fade-in slide-in-from-bottom-4 animate-in rounded-lg border border-primary/50 bg-primary/5 p-6 text-center duration-300"
            id="controlled-step-3"
          >
            <h3 className="font-semibold">Step 3</h3>
            <p className="text-muted-foreground text-sm">Dynamic step that appears after step 2</p>
          </div>
        )}
      </div>
      <Tour
        alignOffset={0} // default 0px offset for alignment (e.g., when align="center", this would be the vertical offset)
        onComplete={onComplete}
        onOpenChange={setOpen}
        onSkip={onSkip}
        onValueChange={onStepChange}
        open={open}
        sideOffset={16} // default 16px gap between the target and the tooltip
        stepFooter={
          <TourFooter>
            <div className="flex w-full items-center justify-between">
              <TourStepCounter />
              <div className="flex gap-2">
                <TourPrev />
                <TourNext />
              </div>
            </div>
          </TourFooter>
        }
        value={value}
      >
        <TourSpotlight />
        <TourSpotlightRing />
        <TourStep align="center" side="bottom" target="#controlled-title">
          <TourArrow />
          <TourHeader>
            <TourTitle>Controlled Tour</TourTitle>
            <TourDescription>
              This tour's state is controlled externally. Notice how the step counter updates.
            </TourDescription>
          </TourHeader>
          <TourClose />
        </TourStep>
        <TourStep align="center" side="top" target="#controlled-step-1">
          <TourArrow />
          <TourHeader>
            <TourTitle>External Controls</TourTitle>
            <TourDescription>
              You can control this tour using the external buttons above, or use the built-in
              navigation.
            </TourDescription>
          </TourHeader>
          <TourClose />
        </TourStep>
        <TourStep align="center" side="top" target="#controlled-step-2">
          <TourArrow />
          <TourHeader>
            <TourTitle>Second Feature</TourTitle>
            <TourDescription>
              The tour state is fully controlled by the parent component. Watch what happens next!
            </TourDescription>
          </TourHeader>
          <TourClose />
        </TourStep>
        <TourStep align="center" side="top" target="#controlled-step-3">
          <TourArrow />
          <TourHeader>
            <TourTitle>Dynamic Layout</TourTitle>
            <TourDescription>
              This element appeared when you reached this step, demonstrating how the tour handles
              dynamic content and layout shifts.
            </TourDescription>
          </TourHeader>
          <TourClose />
        </TourStep>
      </Tour>
    </div>
  );
}
